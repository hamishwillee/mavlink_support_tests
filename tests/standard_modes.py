...
class StandardModesTest:
    def __init__(
        ...
        self.commander = command_sender.CommandSender(
            ...
            own_component_id=self.own_component_id,
        )
    ...
    def getSupportedModes(self):
        ...
        targetComponent = self.message_set.enum(
    ...
    def supported_modes(self, msg):
        ...
        if messageName == "CURRENT_MODE":
            ...
            if self._current_mode_last_timestamp:
                ...
                self.current_mode_rate_avg = (
                    self._current_mode_period_count
                    / self._current_mode_accumulated_time
                )
        ...
        if messageName == "AVAILABLE_MODES_MONITOR":
            ...
            if self._modes_monitor_last_timestamp:
                ...
                self._available_modes_monitor_rate_avg = (
                    self._modes_monitor_period_count
                    / self._modes_monitor_accumulated_time
                )
    ...
    def __report(self):
        ...
        if self._available_modes_monitor_rate_avg:
            print(
                f"AVAILABLE_MODES_MONITOR streaming rate (Hz) =  {self._available_modes_monitor_rate_avg}"
            )
        else:
        ...
        if self.duplicateAvailableModes:
            print(
                f"PROBLEM - AVAILABLE_MODE with same index sent twice - maybe not - this could be result of my follow up test"
            )
