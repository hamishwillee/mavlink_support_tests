...
class MAVConnection:
    def __init__(
        self,
        connection_type="px4wsl2_companion_udp_server",
        own_system_id=250,
        own_component_id=194,
        dialect="development",
    ):
        ...
        self.conn_runtime = libmav.NetworkRuntime(
            libmav.Identifier(self.own_system_id, self.own_component_id),
            self.message_set,
            heartbeat_message,
            conn_physical,
        )
    ...
    def _threaded_callback_wrapper(self, callback_func, stop_event):
        ...
        try:
            ...
            mav_callback_handle = self.connection.add_message_callback(callback_func)
            print(
            ...
            print(f"ERROR in callback thread for {callback_func.__name__}: {e}")
        finally:
            print(f"Debug: Callback thread for '{callback_func.__name__}' cleaning up.")
            if self.connection and mav_callback_handle is not None:
                try:
                    self.connection.remove_message_callback(mav_callback_handle)
                    print(
                        f"Debug: MAVLink callback {mav_callback_handle} removed for '{callback_func.__name__}'."
                    )
                except Exception as e:
                    print(f"Error removing MAVLink callback {mav_callback_handle}: {e}")
    ...
    def remove_threaded_message_callback(self, callback_id):
        ...
        if callback_id not in self._threaded_callbacks:
            print(f"Warning: No threaded callback found with ID: {callback_id}")
            return False

        print(f"MAVConnection: Signalling threaded callback {callback_id} to stop...")
    ...
    def _messageArrived(self, msg):
        ...
        comp_type = self.docs.getEnumEntryNameFromId("MAV_TYPE", message_dict["type"])
