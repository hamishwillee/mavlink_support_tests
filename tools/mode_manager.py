...
# modes list
class StandardModes:
    def __init__(self, connection, mavlinkDocs, libmav_message_set, target_system, target_component):
        ...
        self.commander = command_sender.CommandSender(connection=self.connection, mavlinkDocs=self.docs, libmav_message_set=self.message_set,
    ...
    def requestModes(self):
        ...
        def getModesTimeout(self):
            ...
            for key in missing_keys:
                ...
                self.commander.sendCommandRequestMessageNonBlocking(
                    target_system=self.target_system, target_component=self.target_component, request_message_id=request_message_id, index_id=key, callback=None)
        ...
        if check_value(self.supports_standard_modes_api):
            ...
            self.timerGetModes = threading.Timer(
                5.0, getModesTimeout, args=[self])
            self.timerGetModes.start()

            get_all_modes = 0
            self.commander.sendCommandRequestMessageNonBlocking(
                target_system=self.target_system, target_component=self.target_component, request_message_id=request_message_id, index_id=get_all_modes, callback=getModesCallback)
    ...
    def standard_mode_listener(self, msg):
        ...
        if messageName == 'AVAILABLE_MODES_MONITOR':
            ...
            if self.available_modes_seq is None:
                ...
                print(f"Debug: AVAILABLE_MODES_MONITOR sequence number changed: {self.available_modes_seq} -> {message_dict['seq']}")
                self.available_modes_seq = message_dict['seq']
                # Re-request modes to ensure we have the latest.
                print("Re-requesting modes due to AVAILABLE_MODES_MONITOR sequence change.")
