...
class MAVComponent:
    def __init__(
        self, mav_connection, target_system_id, target_component_id, mav_type, autopilot
    ):
        """
    ...
    def isCapabilitySupported(self, capability):
        ...
        if self.msg_autopilot_version and "capabilities" in self.msg_autopilot_version:
            ...
            caps = self.mav_connection.docs.getEnumEntriesId("MAV_PROTOCOL_CAPABILITY")
            pprint.pprint(caps)
            capability
    ...
    def _handle_autopilot_version(self, message_dict):
        ...
        def getProtocolsSupported(capabilities):
            ...
            protocols = dict()
            for key, capability in self.docs.getEnum(name="MAV_PROTOCOL_CAPABILITY")[
                "entries"
            ].items():
        ...
        self.msg_autopilot_version["capabilities"] = getProtocolsSupported(
            message_dict["capabilities"]
        )
        ...
        self.msg_autopilot_version["flight_custom_version"] = customVersion(
            message_dict["flight_custom_version"]
        )
        self.msg_autopilot_version["os_custom_version"] = customVersion(
            message_dict["os_custom_version"]
        )
        self.msg_autopilot_version["middleware_custom_version"] = customVersion(
            message_dict["middleware_custom_version"]
        )
